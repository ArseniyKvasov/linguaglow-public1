...
if (userRole === 'teacher' && mode === 'generation') {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function enableDragAndDrop() {
            ...
            if (!taskList.sortableInstance) {
                taskList.sortableInstance = new Sortable(taskList, {
                    ...
                    onStart: function(evt) {
                        ...
                        // текущий элемент сделать полностью видимым
                        evt.item.style.opacity = 1;
                        // при старте перетаскивания дополнительно подсветим (опционально)
            ...
            // Ограничиваем высоту КАЖДОГО задания при включении режима
            document.querySelectorAll('.task-item').forEach((task) => {
                task.style.maxHeight = '200px';
                task.style.overflowY = 'auto';
                // чуть плавности
                task.style.transition = 'max-height 0.15s ease';
            });
        }
        ...
        function disableDragAndDrop() {
            ...
            // Возвращаем задания к исходному стилю
            document.querySelectorAll('.task-item').forEach((task) => {
                task.style.maxHeight = '';
                task.style.overflowY = '';
                task.style.transition = '';
            });

            // на всякий случай убираем вспомогательные классы
            document.querySelectorAll('.task-item.dragging-active').forEach(el => el.classList.remove('dragging-active'));
        }
