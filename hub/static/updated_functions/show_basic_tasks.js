...
function showLoadingOverlay() {
    ...
    const phraseEl = document.getElementById('pdfLoadingText');
    ...
    if (phraseEl) phraseEl.textContent = loadingPhrases[0];
    if (progressEl) progressEl.textContent = 'Занимает до 5 минут';

    // обновляем фразу каждые 5 секунд
    phraseInterval = setInterval(() => {
        const currentIndex = loadingPhrases.indexOf(phraseEl.textContent);
        const nextIndex = (currentIndex + 1) % loadingPhrases.length;
        if (phraseEl) phraseEl.textContent = loadingPhrases[nextIndex];
    }, 5000);
}
...
// ========== Упрощенные прогресс-утилиты (если нужны для логики) ==========
let totalTasks = 0;
...
async function registerPdfDownload() {
    try {
        const response = await fetch("/api/pdf/downloaded/", {
            method: "POST",
            headers: {
                "X-CSRFToken": getCookie("csrftoken"),
                "Content-Type": "application/json"
            },
            body: JSON.stringify({}) // можно пустым
        });

        const data = await response.json();
        if (data.status === "ok") {
            console.log("PDF download registered:", data.pdf_downloaded_counter);
        } else {
