...
function renderContent(content, format = "html") {
    switch (format.toLowerCase()) {
        case "markdown":
        case "md":
            return marked.parse(content); // markdown → html
        case "text":
        case "plain":
            return content
                .replace(/&/g, "&amp;")
        ...
        case "html":
        default:
            return content; // уже готовый HTML
    }
}

function handleNote(task_id, payloads) {
    const { id, title, content, format = "html" } = payloads;
    const taskContainer = document.getElementById(id);
    if (!taskContainer) return;
...
function renderSectionList() {
    ...
    SECTION_TYPES.forEach((type, typeIndex) => {
        ...
        if (typeIndex > 0 && SECTION_TYPES
                .slice(0, typeIndex)
                .some(t => sections.some(s => s.type === t))) {
        ...
        group.forEach(s => {
            ...
            li.innerHTML =
                `<button type="button"
                        ...
                        data-section-id="${s.id}">${s.name}</button>
                <div class="section-action-buttons align-items-center"
                     style="display: ${userRole==='teacher'?'flex':'none'}; gap: 0.5rem;">
                    <button type="button" class="btn btn-link p-0 edit-section-button" data-section-id="${s.id}" title="Редактировать">
                        <i class="bi bi-pencil-fill text-secondary"></i>
                    </button>
                    <button type="button" class="btn btn-link p-0 delete-btn" data-section-id="${s.id}" title="Удалить">
